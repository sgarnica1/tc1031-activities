...
{
  T aux;
  Node<T> *p; // Crear nueva variable de tipo Node
  p = head;   // Asignar la referencia de head al nodo p

  if (index >= this->size) // Revisar si el índice se encontrará dentro de la lista
  {
...
{
  int aux = index - 1; // Asignar el valor del index - 1
  Node<T> *p, *q;      // Crear dos variables tipo Node

  if (empty()) // Revisar si existen elementos en la lista, sino agregar al principio
  {
    push_front(val);
    return;
  }
...
{
  T aux = index - 1; // Asignar el valor del index - 1
  Node<T> *p, *q;    // Crear dos variables tipo Node
  ...
  if (index == 0) // Revisar si el índice es igual a cero
  {
    head = head->next; // Asignar la referencia del segundo valor de la lista al head, olvidarse del primero
    size--;            // Reducr el tamaño de la lista
    return p->value;   // Regresar el valor del segundo nodo en la lista
  }
  ...
  q = p->next;       // Guardar el atributo next del nodo actual / (referncia del siguiente nodo)
  p->next = q->next; // Asignar al atributo next del nodo actual la referencia del valor del atributo next del atributo next del nodo actual
  size--;            // Reducir el tamaño de la lista

  return q->value; // Retornar el valor eliminado
...
{
  Node<T> *p; // Crear nuevo nodo
  p = head;   // Asignar la referencia de head al nodo p para empezar a iterar
  for (long int i = 0; i < size; i++)
  {
    if (p->value == val) // Buscar el valor del nodo que coincida con el valor introducido
    {
      return i;
    }
    p = p->next; // Cambiar la referencia del nodo p al siguiente nodo
  }
