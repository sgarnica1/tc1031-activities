...
{
  // TO DO
  if (this == NULL) // Revisar si existe el nodo raíz
  {
    return 0;
  }
  if (this->left == NULL && this->right == NULL) // Revisar si es un nodo hoja
  {
    return 1;
  }
  else
  {
    return (this->left->leaves() + this->right->leaves()); // En caso contrario, llamar recursivamente la misma función para el nodo izquierdo y derecho
  }
...
{
  // TO DO
  if (this == NULL) // Revisar si existe este nodo
  {
    return 0;
  }
  if (this->left == NULL || this->right == NULL) // Revisar si el nodo tiene hijos para sumar 1 a la profundidad
  {
    return 1;
  }
  else
  {
    uint rightDepth = this->right->depth(); // Obtner de manera recursiva la profundidad de la rama derecha
    uint leftDepth = this->left->depth();   // Obtener de manera recursiva la profundidad de la rama izquierda

    if (leftDepth > rightDepth) // Comparar ambas produndidades y regresar la mayor más 1
    {
      return leftDepth + 1;
    }
    else
...
bool Node<T>::isFull() const
{
  ...
  else if (right == NULL || left == NULL) // Revisar si el nodo tiene hijos
    return true;

  else if (this->right != NULL && this->left != NULL) // Revisar si el nodo tiene ambos hijos
  {
    if (right->depth() == left->depth())       // Revisar si la profunidad de ambos hijos es la misma
      return left->isFull() + right->isFull(); // Encontrar de manera recursiva si estos nodos están completos
  }
...
{
  // TO DO
  if ((left != NULL && left->value == val) || (right != NULL && right->value == val)) // Revisar si existe el nodo izquiero / derecho y si su valor es igual a val
    return value;

  if (val < value && left != NULL) // Si val es menor al valor del nodo, y existe el nodo izquierdo, continuar
    return left->ancestor(val);

  else if (val > value && right != NULL) // Si val es mayor al valor del nodo, y existe el nodo derecho, continuar
